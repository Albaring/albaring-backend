...
public class WebClientConfig {
    ...
    public WebClient webClient() {
        factory.setEncodingMode(
            DefaultUriBuilderFactory.EncodingMode.VALUES_ONLY); // GET 요청의 파라미터 셋팅을 하기 위한 URI 템플릿의 인코딩을 위한 설정
        return WebClient.builder()
            .uriBuilderFactory(factory)
            .codecs(configurer -> configurer.defaultCodecs()
                .maxInMemorySize(2 * 1024 * 1024)) // in-memory buffer 값이 256KB
            .clientConnector(new ReactorClientHttpConnector(
                httpClient)) // 256KB 보다 큰 HTTP 메시지를 처리 시도 -> Throw Exception
            .build();
    }
